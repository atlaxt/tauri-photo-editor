...
import type { BlockEvent, DesignBlockId } from '@cesdk/engine'
...
export class Example {
  ...
  private bindInput(id: string, setter: (_: number) => void) {
    ...
    input?.addEventListener('change', () => {
      if (this.selectedBlock)
        setter(Number.parseFloat(input.value))
    })
  }
  ...
  private updatePropertiesPanel() {
    ...
    if (!this.selectedBlock || !this.engine.block.isValid(this.selectedBlock)) {
      if (selectionInfo)
        selectionInfo.textContent = 'No block selected'
      if (propertyControls)
    ...
    if (selectionInfo)
      selectionInfo.textContent = `Selected: ${blockType}`
    if (propertyControls)
